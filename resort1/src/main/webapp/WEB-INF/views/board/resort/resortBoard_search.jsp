...
<script language="JavaScript">
	function checkSearch() {
		...
		if (searchKeyword.length > 20) {
			...
			form.action = '/resort1/resortBoard_searchNext?search=${boardItem.title}';
			form.submit();
		}
	}
</script>
		...
		<div>
			...
			<table cellspacing="0" width="800" border="1" align="center">
				<tr>
					...
					<td width="480"><p class="p">Title</p></td>
				...
				<c:forEach var="boardItem" items="${pages.content}">
					<tr>
						...
						<td><p class="p">
								<a href="/resort1/resortBoard_view?id=${boardItem.id}">
									<c:choose>
										<c:when test="${fn:length(boardItem.title) > 30}">
						...
						<td><p class="p">${boardItem.writer}</td>
				...
				<div class="pagination">
					<div class="center">
						<c:if test="${pages.number > 0}">
							<a
								href="resortBoard_searchNext?search=${search}&page=${pages.number - 1}">&lt;</a>
						</c:if>

						<c:forEach items="${pageNumbers}" var="i">
							<c:choose>
								<c:when test="${i eq pages.number}">
									<a href="resortBoard_searchNext?search=${search}&page=${i}"
					...
					<table width="800">
						<tr>
							<td style="float: right;"><input type="text" name="search"
								...
								onclick="location.href='/resort1/resortBoard_list'"></td>
						</tr>
					</table>
				</form>
			</div>
