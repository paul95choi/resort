...
<script language="JavaScript">
	function checkSearch() {
		...
		if (searchKeyword.length > 20) {
			...
			form.action = '/resort1/resortBoard_search?search=${boardItem.title}';
			form.submit();
		}
	}
</script>
				...
				<div class="pagination">
					<div class="center">
						<c:if test="${pages.number > 0}">
							<a href="resortBoard_list?page=${pages.number - 1}">&lt;</a>
						</c:if>

						<c:forEach items="${pageNumbers}" var="i">
							<c:choose>
								<c:when test="${i eq pages.number}">
									<a href="resortBoard_list?page=${i}" style="color: red">${i + 1}</a>
								</c:when>
								<c:otherwise>
									<a href="resortBoard_list?page=${i}" style="color: black">${i + 1}</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
					...
					<table width="800">
						<tr>
							<td style="float: right;"><input type="text" name="search"
								...
								onclick="location.href='/resort1/resortBoard_insert'"></td>
						</tr>
					</table>
				</form>
			</div>
